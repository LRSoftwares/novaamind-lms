...
import { fetchWorksheetBySlug, registerCandidate, startSubmission } from '../lib/worksheetApi';
...
export default function WorksheetRegister() {
  ...
  useEffect(() => {
    (async () => {
      ...
      else setWorksheet(result.worksheet);
      setLoading(false);
    })();
  }, [slug]);
  ...
  const handleSubmit = async (e) => {
    ...
    const candResult = await registerCandidate(worksheet.id, form);
    if (candResult.error) {
      setError(candResult.error);
      setSubmitting(false);
      return;
    }

    const subResult = await startSubmission(worksheet.id, candResult.candidate.id);
    if (subResult.error) {
    ...
    if (subResult.alreadySubmitted) {
      navigate(`/worksheet/${slug}/result?submissionId=${subResult.submission.id}`);
      return;
    }

    navigate(`/worksheet/${slug}/fill?submissionId=${subResult.submission.id}`);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-1">{worksheet.title}</h2>
        <p className="text-sm text-gray-500 mb-6">Please share your details to begin</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={submitting} className="w-full bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 transition-colors disabled:opacity-50">
            {submitting ? 'Starting...' : 'Start Worksheet'}
          </button>
        </form>
      </div>
    </div>
