      ...
      <div className="mb-4">
        <div className="flex items-center gap-2 mb-3">
          {['All', ...PROGRAM_TYPES].map(t => {
            const count = t === 'All' ? programs.length : programs.filter(p => p.programType === t).length;
            return (
              <button key={t} onClick={() => setTypeFilter(t)}
                ...
                <span className={`ml-1.5 text-xs ${typeFilter === t ? 'text-blue-200' : 'text-gray-400'}`}>({count})</span>
              </button>
            );
          })}
        </div>
